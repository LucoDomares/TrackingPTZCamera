# usage: task_HIKVISION_camera -i 192.168.178.229 -o 8100 -u usename -p passwd -w "/Users/Lamby/python-onvif-zeep-zeep/wsdl"
# --prototxt deploy.prototxt --model res10_300x300_ssd_iter_140000.caffemodel --confidence 0.50
# --detector detector_classes/model_detector_face_openface_nn4 --embedder openface_nn4.small2.v1.t7
# --recognizer _detectormodels/recognizer.pickle --labelencoder _detectormodels/le.pickle --cameraname "johnny cam"
# --http_port 80 --rtsp_port 8557
...
from camera_classes.HIKVISION_camera import HIKVISION_camera
...
if __name__ == "__main__":
	...
	ap.add_argument("-n", "--cameraname", required=True,
					help="display name of the camera")
	ap.add_argument("-x", "--http_port", required=True,
					help="http port of the camera")
	ap.add_argument("-y", "--rtsp_port", required=True,
					help="rtsp port of the camera")
	...
	camera = HIKVISION_camera(isdebug=True, camera_ip_address=ip_addr, username=username, password=password,
	...
	with Manager() as manager:
		...
		auto_track_enabled = manager.Value("i", 0)
		...
		try:
			...
			# loop indefinitely
			while True:
				if camera.camera is None and not isinstance(camera, base_HTTP_camera):
					continue
